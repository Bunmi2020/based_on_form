    ...
    <div className="top_header" ref={topMenuRef}>
      ...
      <NavLink  to="/17-20_Jan" 
          ...
          onClick={() => handleButtonClick('older-week')}>
            <button className={activeButton === 'older-week' ? 'active' : ''}>17-20 Jan.</button>
      </NavLink>
      <NavLink  to="/24-27_Jan" 
          className="navbar__menu"
          onClick={() => handleButtonClick('old-week')}>
            <button className={activeButton === 'old-week' ? 'active' : ''}>24-27 Jan.</button>
      </NavLink>
      <NavLink  to="/31-3_Feb" 
          className="navbar__menu"
          onClick={() => handleButtonClick('previous-week')}>
            <button className={activeButton === 'previous-week' ? 'active' : ''}>31-3 Feb.</button>
      </NavLink>
      <NavLink  to="/7-10_Feb" 
          className="navbar__menu"
          onClick={() => handleButtonClick('last-week')}>
            <button className={activeButton === 'last-week' ? 'active' : ''}>7-10 Feb.</button>
      </NavLink>
      <NavLink  to="/14-17_Feb" 
          className="navbar__menu"
          onClick={() => handleButtonClick('current-week')}>
            <button className={activeButton === 'current-week' ? 'active' : ''}>14-17 Feb.</button>
      </NavLink>
